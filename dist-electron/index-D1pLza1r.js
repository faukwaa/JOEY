...
import require$$0$1, { dirname, join } from "path";
import require$$0$4, { existsSync, stat, readdir, rm, readFileSync, writeFileSync, mkdirSync } from "fs";
import { fileURLToPath as fileURLToPath$1 } from "url";
import { exec } from "child_process";
import require$$0$2, { promisify as promisify$1 } from "util";
...
function requirePattern() {
  ...
  pattern.isNegativePattern = isNegativePattern2;
...
function requireOut() {
  ...
  (function(FastGlob2) {
    ...
    (function(posix) {
      ...
      posix.convertPathToPattern = convertPathToPattern3;
    ...
    (function(win32) {
      ...
      win32.convertPathToPattern = convertPathToPattern3;
...
isTypeSync.bind(void 0, "statSync", "isDirectory");
...
function requireIgnore() {
  ...
  (function(module) {
    ...
    const makeRegexPrefix = (pattern2) => REPLACERS.reduce(
    ...
    const splitPattern = (pattern2) => pattern2.split(REGEX_SPLITALL_CRLF).filter(Boolean);
    class IgnoreRule {
      constructor(pattern2, mark, body, ignoreCase, negative, prefix) {
...
const gitIgnore = /* @__PURE__ */ getDefaultExportFromCjs(ignoreExports);
...
const applyBaseToPattern = (pattern2, base) => isNegativePattern(pattern2) ? "!" + path$1.posix.join(base, pattern2.slice(1)) : path$1.posix.join(base, pattern2);
...
const isIgnoredByIgnoreFiles = async (patterns, options) => {
  const { cwd, suppressErrors, deep: deep2, ignore: ignore2 } = normalizeOptions$1(options);
...
const normalizePathForDirectoryGlob = (filePath, cwd) => {
  ...
  return path$1.isAbsolute(path2) ? path2 : path$1.join(cwd, path2);
};
const getDirectoryGlob = ({ directoryPath, files, extensions }) => {
  const extensionGlob = extensions?.length > 0 ? `.${extensions.length > 1 ? `{${extensions.join(",")}}` : extensions[0]}` : "";
  return files ? files.map((file) => path$1.posix.join(directoryPath, `**/${path$1.extname(file) ? file : `${file}${extensionGlob}`}`)) : [path$1.posix.join(directoryPath, `**${extensionGlob ? `/*${extensionGlob}` : ""}`)];
};
...
const createFilterFunction = (isIgnored) => {
  ...
  return (fastGlobResult) => {
    const pathKey = path$1.normalize(fastGlobResult.path ?? fastGlobResult);
...
function isPathInside(childPath, parentPath) {
  ...
  return Boolean(
    relation && relation !== ".." && !relation.startsWith(`..${path$1.sep}`) && relation !== path$1.resolve(childPath)
  );
}
async function trash(paths, options = {}) {
  ...
  if (glob) {
    const patterns = paths.map((pattern2) => {
      const isNegated = pattern2.startsWith("!");
      const body = isNegated ? pattern2.slice(1) : pattern2;
      const normalized = body.endsWith("/**") ? path$1.dirname(body) : body;
    ...
    const literals = await Promise.all(paths.map(async (p) => {
      try {
        await fs$4.promises.lstat(p);
        return p;
      } catch {
        return null;
      }
    }));
    paths = [.../* @__PURE__ */ new Set([...matches, ...literals.filter(Boolean)])];
  }
  const pathChecks = await Promise.all(paths.map(async (filePath) => {
    ...
    try {
      await fs$4.promises.lstat(filePath);
...
async function platformSpecificImplementation() {
  switch (process$1.platform) {
    case "darwin": {
      return import("./macos-DCqTSJ5C.js");
    }
    case "win32": {
      return import("./windows-D8hxmrRh.js");
    }
    default: {
      return import("./linux-D3X6NFqO.js");
    }
  }
}
...
process.env.DIST = require$$0$1.join(__dirname$1, "../dist");
process.env.VITE_PUBLIC = app.isPackaged ? process.env.DIST : require$$0$1.join(__dirname$1, "../../public");
let win = null;
const getConfigPath = () => {
...
const getScanFolders = () => {
  try {
    ...
    console.error("Error reading scan folders config:", error2);
  }
  return [];
};
const saveScanFolders = (folders) => {
  try {
    ...
    console.error("Error saving scan folders config:", error2);
    return { success: false, error: String(error2) };
  }
};
const getUserSettingsPath = () => {
  const userDataPath = app.getPath("userData");
  return require$$0$1.join(userDataPath, "config", "user-settings.json");
};
const getUserSettings = () => {
  try {
    ...
    console.error("Error reading user settings:", error2);
  }
  return {};
};
const saveUserSettings = (settings2) => {
  try {
    const settingsPath = getUserSettingsPath();
    const settingsDir = require$$0$1.dirname(settingsPath);
    ...
    console.error("Error saving user settings:", error2);
    return { success: false, error: String(error2) };
  }
};
...
const getProjectsCache = () => {
  try {
    ...
    console.error("Error reading projects cache:", error2);
  }
  return null;
};
const saveProjectsCache = (projects, folders, scannedDirs, folder, favorites, scannedDirsMap) => {
  try {
    const cachePath = getProjectsCachePath();
    ...
    let finalScannedDirsMap = existingCache?.scannedDirsMap || {};
    if (scannedDirsMap) {
    ...
    console.error("Error saving projects cache:", error2);
    return { success: false, error: String(error2) };
  }
};
...
function createWindow() {
  win = new BrowserWindow({
    ...
    minWidth: 800,
    minHeight: 600,
    icon: require$$0$1.join(process.env.VITE_PUBLIC, "favicon.ico"),
    webPreferences: {
      preload: require$$0$1.join(__dirname$1, "preload.cjs"),
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(require$$0$1.join(process.env.DIST, "index.html"));
  }
  win.on("closed", () => {
    win = null;
  });
...
async function scanDirectoryRecursively(dir, projects, scannedDirs, maxDepth = 5, currentDepth = 0, onProgress) {
  ...
  try {
    ...
    for (const entry2 of entries) {
      if (!entry2.isDirectory() || shouldSkipDirectory(entry2.name)) {
        continue;
      }
      const fullPath = join(dir, entry2.name);
      onProgress?.(fullPath);
    ...
    console.error(`Error scanning directory ${dir}:`, error2);
  }
}
ipcMain.handle("scan-projects", async (_event, folders) => {
  ...
  for (const folder of folders) {
    ...
    try {
      ...
      console.error(`Error scanning folder ${folder}:`, error2);
    }
    console.log(`目录扫描完成: ${folder}, 找到 ${projects.length} 个项目`);
  }
  const uniqueProjects = projects.filter(
    (project, index, self2) => index === self2.findIndex((p) => p.path === project.path)
  );
...
ipcMain.handle("open-project-terminal", async (_, projectPath) => {
  try {
    ...
    console.error("Failed to open terminal:", error2);
    return { success: false, error: String(error2) };
  }
});
ipcMain.handle("open-project-vscode", async (_, projectPath) => {
  try {
    ...
    console.error("Failed to open VSCode:", error2);
    return { success: false, error: String(error2) };
  }
});
ipcMain.handle("open-project-qoder", async (_, projectPath) => {
  try {
    ...
    console.error("Failed to open Qoder:", error2);
    return { success: false, error: String(error2) };
  }
});
...
ipcMain.handle("save-user-settings", async (_, settings2) => {
  return saveUserSettings(settings2);
});
ipcMain.handle("save-projects-cache", async (_, projects, folders, scannedDirs, folder, favorites, scannedDirsMap) => {
...
ipcMain.handle("get-project-stats", async (_, projectPath) => {
  try {
    ...
    try {
      const statAsync = promisify$1(stat);
    ...
    try {
      ...
      try {
        ...
        for (const entry2 of entries) {
          if (entry2.isFile()) {
            try {
              const fullPath = join(projectPath, entry2.name);
    ...
    console.error("Error getting project stats:", error2);
    return {
      size: 0,
...
ipcMain.handle("delete-node-modules", async (_, projectPath) => {
  try {
    ...
    const rmAsync = promisify$1(rm);
    await rmAsync(nodeModulesPath, { recursive: true, force: true });
    return { success: true };
  } catch (error2) {
    console.error("Error deleting node_modules:", error2);
    return { success: false, error: String(error2) };
  }
});
ipcMain.handle("delete-project-from-disk", async (_, projectPath) => {
  try {
    ...
    console.error("Error moving project to trash:", error2);
    return { success: false, error: String(error2) };
  }
});
ipcMain.handle("refresh-project-info", async (_, projectPath) => {
  try {
    ...
    console.error("Error refreshing project info:", error2);
    return { success: false, error: String(error2) };
  }
});
export {
